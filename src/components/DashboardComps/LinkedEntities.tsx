...
const LinkedEntities = ({
  ...
  const handleOpen = async (pageToken?: string, isMore: boolean = false) => {
    const getEntities = async () => {
      const url = `/api/getEntities?entityType=${linkedEntityType}${
...
export const createBody = (fieldId: string, entityValues: any): any => {
  const keys = fieldId.split(".");
  let currentObject: any = {};
  let nestedObject: any = currentObject;

  keys.forEach((key, index) => {
    if (index === keys.length - 1) {
      currentObject[key] = entityValues.map((item: any) => item.id);
    } else {
      currentObject[key] = {};
